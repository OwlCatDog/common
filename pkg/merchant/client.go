...
//   - error: 调用失败的错误
func (c *IAMClient) SetTenantPermissions(ctx context.Context, codes []string) (*v1.SetTenantPermissionsResponse, error) {
	if len(codes) == 0 {
		return nil, fmt.Errorf("权限代码列表不能为空")
	}

	resp, err := c.client.SetTenantPermissions(ctx, &v1.SetTenantPermissionsRequest{Codes: codes})
	if err != nil {
